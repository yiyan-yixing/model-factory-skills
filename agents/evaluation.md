...
- **位置**：🚦 闸门（独立）
- **说明**：评测不隶属于创造环或推广环，有发布否决权。这是结构性要求——评测独立不能靠文化，必须靠架构（OpenAI 教训：独立评测团队被生产压力侵蚀解散）

## 能力成长区

- 方法论沉淀：评测基准构建方法、AB 测试设计模板、安全红队用例库 → archival/evaluation/
- 知识资产：评测基准库、红队测试集、评测报告模板
- 改进方向：评测覆盖率、自动指标与人工判断的一致性、AB 测试结论准确度
