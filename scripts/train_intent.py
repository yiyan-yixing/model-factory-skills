...
- P0: Data no longer pre-loaded to MPS; __getitem__ moves tensors to device on-the-fly
- P1: Added torch.mps.empty_cache() for MPS memory management
- P2: Added KeyboardInterrupt handling for graceful shutdown

Bug fixes (2026-07-15):
- Bug1 (P1): Best checkpoint now saved to separate best_model/ dir;
  final epoch checkpoint saved to last_checkpoint/. Previously, the final
  save overwrote the best checkpoint, meaning the eval script loaded the
  last-epoch model, not the best-eval-loss model. Output now targets v2 dir.
- Bug2 (P1): torch.no_grad() replaced with torch.inference_mode() in
  eval sections for better performance and correctness.

Post-review fixes (2026-07-15):
- P1: final eval now participates in best-checkpoint tracking
- P1: best_checkpoint_path set to None when no eval was ever triggered
- P2: final eval uses same 50-sample cap as training eval for comparability
"""
...
V2_DIR = os.path.join(PROJECT_DIR, "models", "intent-0.5b-v2")
BEST_CHECKPOINT_DIR = os.path.join(V2_DIR, "best_model")
LAST_CHECKPOINT_DIR = os.path.join(V2_DIR, "last_checkpoint")
...
LOG_EVERY = 10
EVAL_EVERY = 30
EVAL_MAX_SAMPLES = 50  # Cap eval samples for speed; used in both training eval and final eval
WARMUP_RATIO = 0.1
...
class IntentSFTDataset(TorchDataset):
    ...
    This avoids pinning all data to MPS memory at init time (P0 fix).
    """
...
def main():
    ...
    # Data stored on CPU; moved to device in __getitem__ (P0 fix)
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                if (batch_idx + 1) % GRAD_ACCUM == 0:
                    ...
                    if global_step % EVAL_EVERY == 0:
                        ...
                        eval_loss = 0
                        n = 0
                        # Bug2 fix: use inference_mode instead of no_grad
                        ...
                        # Bug1 fix: save best checkpoint to separate best_model/ dir
                        flag = ""
    ...
    # Final eval (same sample cap as training eval for comparability)
    model.eval()
    eval_loss = 0
    n = 0
    # Bug2 fix: use inference_mode instead of no_grad
    ...
    # Post-review fix: final eval participates in best-checkpoint tracking
    if eval_loss < best_eval_loss:
        best_eval_loss = eval_loss
    ...
    metrics_path = os.path.join(V2_DIR, "training_metrics.json")
